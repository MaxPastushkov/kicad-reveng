...
BOARD_CONNECTED_ITEM* PNS_KICAD_IFACE::createBoardItem( PNS::ITEM* aItem )
{
    ...
    // Tracks with net code <= 0 will get auto-generated nets assigned during connectivity propagation
    if( net->GetNetCode() <= 0 )
    {
...
void PNS_KICAD_IFACE::AddItem( PNS::ITEM* aItem )
{
    ...
    if( boardItem )
    {
        ...
        // Track unconnected tracks/vias for auto-net generation
        if( boardItem->GetNetCode() == 0 )
        {
            m_addedUnconnectedTracks.push_back( boardItem );
        }
    }
}
...
void PNS_KICAD_IFACE::Commit()
{
    ...
    // For PCB-only workflow: if we routed any unconnected tracks, determine the appropriate net
    if( !m_addedUnconnectedTracks.empty() )
    {
        wxLogTrace( wxT( "PNS" ), wxT( "Processing %zu unconnected tracks for auto-net assignment" ),
                    m_addedUnconnectedTracks.size() );

        // First push the commit to add the items to the board
        m_commit->Push( _( "Routing" ), m_commitFlags );
        m_commit = std::make_unique<BOARD_COMMIT>( m_tool );

        // Get the connectivity data to analyze what we connected to
        std::shared_ptr<CONNECTIVITY_DATA> connectivity = m_board->GetConnectivity();

        // Rebuild connectivity to detect physical connections
        wxLogTrace( wxT( "PNS" ), wxT( "Rebuilding connectivity..." ) );
        connectivity->Build( m_board );

        // Find all nets that our routed items physically connect to
        std::set<int> connectedNets;

        for( BOARD_CONNECTED_ITEM* track : m_addedUnconnectedTracks )
        {
            // Get all items physically connected to this track, ignoring net assignments
            std::vector<BOARD_CONNECTED_ITEM*> connectedItems =
                connectivity->GetConnectedItems( track, IGNORE_NETS );

            wxLogTrace( wxT( "PNS" ), wxT( "Track found %zu connected items (ignoring nets)" ),
                        connectedItems.size() );

            for( BOARD_CONNECTED_ITEM* item : connectedItems )
            {
                int itemNet = item->GetNetCode();
                wxLogTrace( wxT( "PNS" ), wxT( "  Connected item type=%d net=%d" ),
                            item->Type(), itemNet );
                // Collect all non-unconnected nets
                if( itemNet > 0 )
                {
                    connectedNets.insert( itemNet );
                }
            }
        }

        wxLogTrace( wxT( "PNS" ), wxT( "Found %zu unique nets connected" ), connectedNets.size() );

        NETINFO_ITEM* targetNet = nullptr;

        if( connectedNets.size() > 1 )
        {
            // Connected to multiple different nets - this is a short circuit
            // For now, just use the first net found (KiCad's DRC will flag this)
            targetNet = m_board->GetNetInfo().GetNetItem( *connectedNets.begin() );
            wxLogTrace( wxT( "PNS" ), wxT( "Multiple nets found - using net %d (%s)" ),
                        targetNet->GetNetCode(), targetNet->GetNetname() );
        }
        else if( connectedNets.size() == 1 )
        {
            // Connected to exactly one existing net - use it
            targetNet = m_board->GetNetInfo().GetNetItem( *connectedNets.begin() );
            wxLogTrace( wxT( "PNS" ), wxT( "Using existing net %d (%s)" ),
                        targetNet->GetNetCode(), targetNet->GetNetname() );
        }
        else
        {
            // Not connected to any existing nets - create a new auto-generated net
            targetNet = const_cast<NETINFO_LIST&>( m_board->GetNetInfo() ).CreateAutoGeneratedNet();
            wxLogTrace( wxT( "PNS" ), wxT( "Created new auto-generated net %d (%s)" ),
                        targetNet->GetNetCode(), targetNet->GetNetname() );
        }

        if( targetNet )
        {
            wxLogTrace( wxT( "PNS" ), wxT( "Assigning net %d to %zu tracks" ),
                        targetNet->GetNetCode(), m_addedUnconnectedTracks.size() );

            // Assign the net to all routed items
            for( BOARD_CONNECTED_ITEM* track : m_addedUnconnectedTracks )
            {
                m_commit->Modify( track );
                track->SetNet( targetNet );
                // Update connectivity for this track
                connectivity->Update( track );
            }

            // Propagate the net to physically connected items (like pads) using the same commit
            connectivity->PropagateNets( m_commit.get() );

            // Commit both the net assignments and the propagated changes
            wxLogTrace( wxT( "PNS" ), wxT( "Committing net assignments..." ) );
            m_commit->Push( _( "Assign nets" ), m_commitFlags );
            m_commit = std::make_unique<BOARD_COMMIT>( m_tool );

            // Rebuild ratsnest to show the new connections
            wxLogTrace( wxT( "PNS" ), wxT( "Recalculating ratsnest..." ) );
            connectivity->RecalculateRatsnest( nullptr );
            wxLogTrace( wxT( "PNS" ), wxT( "Auto-net assignment complete" ) );
        }
    }
    else
    {
        // No unconnected tracks - just push the normal commit
        m_commit->Push( _( "Routing" ), m_commitFlags );
        m_commit = std::make_unique<BOARD_COMMIT>( m_tool );
    }

    m_addedUnconnectedTracks.clear();
}
...
PNS::NET_HANDLE PNS_KICAD_IFACE_BASE::CreateAutoGeneratedNet()
{
    if( !m_board )
        return GetOrphanedNetHandle();

    // GetNetInfo() returns const reference, but we need to modify the net list
    // to create a new net. We use const_cast here because we're intentionally
    // modifying the board state during routing.
    NETINFO_LIST& netInfo = const_cast<NETINFO_LIST&>( m_board->GetNetInfo() );
    NETINFO_ITEM* newNet = netInfo.CreateAutoGeneratedNet();

    // If net creation failed, fall back to orphaned net
    if( !newNet )
        return GetOrphanedNetHandle();
