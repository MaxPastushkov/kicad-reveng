...
NETINFO_ITEM* NETINFO_LIST::CreateAutoGeneratedNet()
{
    // Safety check: ensure we have a valid parent board
    if( !m_parent )
        return nullptr;

    // Get the next available net code
    int newNetCode = getFreeNetCode();

    // Create a unique name in the format "Net-(PCB-N)"
    wxString netName = wxString::Format( wxT( "Net-(PCB-%d)" ), newNetCode );

    // Ensure the name is unique (in case there's a conflict)
    int suffix = 1;
    wxString baseName = netName;
    while( GetNetItem( netName ) != nullptr )
    {
        netName = wxString::Format( wxT( "%s_%d" ), baseName, suffix++ );
    }

    // Create the new net item
    // Note: netclass will be set later via BOARD::SynchronizeNetsAndNetClasses()
    NETINFO_ITEM* newNet = new NETINFO_ITEM( m_parent, netName, newNetCode );
